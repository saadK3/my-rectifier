...
if __name__ == '__main__':
    ...
    print(f"Rectifying {image_name}")
    # Run the independent algorithm (no focal required)
    warped = rectify_image(image_name, clip_factor=4, algorithm='independent')
    # Convert float64 [0..1] to uint8 [0..255] so skimage.io.imsave can handle it
    warped_uint8 = (np.clip(warped, 0, 1) * 255).astype(np.uint8)
    save_name = '.'.join(image_name.split('.')[:-1]) + '_warped.png'
    io.imsave(save_name, warped_uint8)
    print("Saved rectified image to", save_name)
